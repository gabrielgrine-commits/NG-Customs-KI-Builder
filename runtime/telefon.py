...
def synthflow_werkzeug(agent: Agent, name: str, body: dict) -> dict:
    ...
    props = TOOL_DEFINITIONEN[name]["input_schema"]["properties"]
    try:  # erst Text-Variablen angleichen ("" → null), dann Pflichtfelder prüfen
        eingabe = _auffuellen(name, _typen_angleichen(name, {f: w for f, w in body.items() if f in props}))
    except ValueError as e:
        return {"fehler": str(e)}
    inhalt, _, _ = agent.werkzeug_ausfuehren(name, eingabe, "telefon")
